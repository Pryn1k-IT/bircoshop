...
//fix for one el in row
$(document).on({
	mouseenter: function () {
	var height = 0;
	if($(this).next('.product').length > 0){
	height = $(this).next('.product').height();
	}
	if(height == 0 && $(this).prev('.product').length > 0){
	height = $(this).prev('.product').height();
	}
	if(height == 0){
	height = parseInt($(this).find('.inner').innerHeight(), 10);
	marg_height = $(this).find('.product__buy').css('margin-top');
	height = height - parseInt(marg_height, 10);
	//console.log(height);
	}
	if(height == 0){
