...
var gulpv = 4,
	...
	plugin_src = '../catalog/view/javascript/',
	allowed_css_files = [
		//'catalog/view/theme/default/stylesheet/admin-bar.css',
	],
	allowed_js_files = [
		//'catalog/view/javascript/progroman/jquery.progroman.city-manager.js',
	];
...
function styles(){
	...
	.pipe(filesize())
	// .pipe(rename(output_css_name+debag_text+'.css'))
...
function write_new_timestamp(){
	...
	var file_content = "<?php\ndefine('DEBUG_TEXT', '"+add_text+"');\n";
		file_content += "define('STYLES_TIMESTAMP', '"+(time_range)+"');\n";
...
gulp.task('deploy', gulp.series(scripts_uglify, write_new_timestamp));
